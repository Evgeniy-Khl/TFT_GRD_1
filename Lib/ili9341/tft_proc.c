      ...
      case 3://--------- ЗМІНА ТЕМПЕРАТУР -------------------------
        switch (item){
          ...
          case 3: 
            ...
            ILI9341_WriteString(60, Y_top+60, "ВИКОНАЮ ЗАПИС!", Font_11x18, ILI9341_GREEN, ILI9341_BLACK);
            set[numSet] = newval[numSet];     // установим новые значения
            uint32_t er = writeData();        // запишем значения во FLASH
            if(er) ILI9341_WriteString(100, Y_top+90, "ПОМИЛКА!", Font_11x18, ILI9341_YELLOW, ILI9341_RED);
      ...
      case 4://--------- ЗМІНА РЕЖИМУ ----------------------------------
        switch (item){
          ...
          case 3: 
            ...
            ILI9341_WriteString(60, Y_top+60, "ВИКОНАЮ ЗАПИС!", Font_11x18, ILI9341_GREEN, ILI9341_BLACK);
            mode = newval[0];
            setData(mode);                    // установим новые значения
            uint32_t er = writeData();        // запишем значения во FLASH
            if(er) ILI9341_WriteString(100, Y_top+90, "ПОМИЛКА!", Font_11x18, ILI9341_YELLOW, ILI9341_RED);
            else ILI9341_WriteString(120, Y_top+90, "OK", Font_11x18, ILI9341_GREEN, ILI9341_BLACK);
            HAL_Delay(1000);
