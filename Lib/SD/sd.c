...
//--------------------------------------------------
extern char buffTFT[30];
extern uint8_t Y_txt, X_left, checkTime, card;
extern SPI_HandleTypeDef hspi1;
extern volatile uint16_t fillScreen, Timer1;
sd_info_ptr sdinfo;
//--------------------------------------------------
...
{
  checkTime = 0;
  while(checkTime<2); //ждём 20 милисекунд, для того, чтобы напряжение стабилизировалось
  ILI9341_WriteString(X_left, Y_txt, "SD PowerOn", Font_7x10, ILI9341_GREEN, fillScreen);
  Y_txt = Y_txt+10+2;
}
*/
//-----------------------------------------------
...
uint8_t sd_ini(void)
{
  ...
  sdinfo.type = 0;
	...
	hspi2.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_256; //281.25 kbbs
	HAL_SPI_Init(&hspi1);
	SS_SD_DESELECT();
...
//			sprintf(buffTFT,"OCR: 0x%02X 0x%02X 0x%02X 0x%02X",ocr[0],ocr[1],ocr[2],ocr[3]);
...
//    ILI9341_WriteString(X_left, Y_txt, "SD карта не вставлена!", Font_11x18, ILI9341_MAGENTA, fillScreen);
//    Y_txt = Y_txt+18+5;
    return 1;
  }
//  sprintf(buffTFT,"SD карта определена: 0x%02X",sdinfo.type);
//  ILI9341_WriteString(X_left, Y_txt, buffTFT, Font_11x18, ILI9341_GREEN, fillScreen);
//  Y_txt = Y_txt+18+5;
//  card = 1;
  return 0;
}
